...
namespace timw255.FormAnalytics.Data.EntityFramework.Decorators
{
    public class FormAnalyticsEFDataProviderDecorator : IDataProviderDecorator, IExecutionStateDecorator
    {
        ...
        public void Initialize(string providerName, NameValueCollection config, Type managerType)
        {
            ...
            var provider = this.DataProvider as IFormAnalyticsEFDataProvider;

            if (provider != null)
            {
                FormAnalyticsEFDataConnection connection = FormAnalyticsEFDataConnection.InitializeConnection(this.connectionName, provider);

                provider.ProviderContext = new FormAnalyticsEFDataProviderContext()
                {
        ...
        {
            var provider = (IFormAnalyticsEFDataProvider)this.DataProvider;
        ...
        {
            var dataProvider = (IFormAnalyticsEFDataProvider)this.DataProvider;
        ...
        /// <returns></returns>
        private FormAnalyticsEFDbContext GetContext(IFormAnalyticsEFDataProvider provider)
        {
            if (provider.ProviderContext == null)
                throw new Exception(String.Format("Provider {0} is not initialized.", provider.Name));

            return FormAnalyticsEFDataConnection.GetContext(this.connectionName, provider);
        }
        ...
        /// <returns></returns>
        private string GetScopeId(IFormAnalyticsEFDataProvider provider)
        {
